...
let enviarDatos = async () => {
	...
	obtenerArchivo("id:nAjSo8FkJMAAAAAAAAAAGA");
	 return false;
	
	const nombre = obtenerFechaInt();
    const nombreFinal = nombre + '_' + codificar(nombre)+'.'+obtenerExtension(InputFotoCentroEdu.files[0].name);
	
    let fotoCentro1 = await guardarAdjunto(InputFotoCentroEdu.files[0], nombreFinal) || {};
    console.log("await: ", fotoCentro1);

/*
    let v = {"name": "20190321173315_.jpg", "path_lower": "/20190321173315_.jpg", "path_display": "/20190321173315_.jpg", "id": "id:nAjSo8FkJMAAAAAAAAAAGA", "client_modified": "2019-03-21T23:33:17Z", "server_modified": "2019-03-21T23:33:17Z", "rev": "013000000013462ecb0", "size": 94309, "content_hash": "bcb4c04d9a6b13229987b7cfc33a108c37823af809985a148977b69ec23910c6"};
*/

const linkDropbox = 'https://dl.dropboxusercontent.com/1/view/1628nt8i6lbz6pp/Aplicaciones/JamsApp/'+fotoCentro1.name;


const linkDropbox2 = 'https://dl.dropboxusercontent.com/1/view/g08vb2jco4v08o4/Aplicaciones/JamsApp/20190321173315_.jpg'+fotoCentro1.name;


mostrarAlerta('Descargar ('+tamannoArchivo(fotoCentro1.size)+'): '+linkDropbox);
        return false;
    ...
    /*
        ...
        formulario.append('nombreComercial', NombreComercial);
        formulario.append('cedulaJuridica', CedulaJuridica);
        formulario.append('correoCentro', CorreoCentro);
        formulario.append('annoFundacion', AnnoFundacion);
        formulario.append('resenna', Resenna);
        formulario.append('telefonoCentro', TelefonoCentro);
        formulario.append('fax', Fax);
        formulario.append('tipoInstitucion', TipoInstitucion);
        formulario.append('niveles', Niveles);
        formulario.append('idProvincia', IdProvincia);
        formulario.append('idCanton', IdCanton);
        formulario.append('idDistrito', IdDistrito);
        formulario.append('dirSennas', DirSennas);
        formulario.append('primerNombre', PrimerNombre);
        formulario.append('segundoNombre', SegundoNombre);
        formulario.append('primerApellido', PrimerApellido);
        formulario.append('segundoApellido', SegundoApellido);
        formulario.append('correoContacto', CorreoContacto);
        formulario.append('identificacionContacto', IdentificacionContacto);
        formulario.append('departamentoContacto', DepartamentoContacto);
        formulario.append('telefonoContacto', TelefonoContacto);


        if (InputFotoCentroEdu.files && InputFotoCentroEdu.files[0]) {

            //Si existe una foto: la limitamos a que sea menor a 2mb.
            // 2mb = 2097152 Bytes

            if (InputFotoCentroEdu.files[0].size > 2097152) {
                mostrarAlerta('No se permiten fotos mayores a 2 MB', InputFotoCentroEdu);
                return false;
            } else {
                formulario.append("fotoCentro[]", InputFotoCentroEdu.files[0], InputFotoCentroEdu.files[0].name);
            }
        }

    registrarCentroEducativo(formulario);
    */
    ...
    if (InputFotoCentroEdu.files && InputFotoCentroEdu.files[0]) {
        // 200 KB = 204800 Bytes
        if (InputFotoCentroEdu.files[0].size > 204800) {
            mostrarAlerta('No se permiten fotos mayores a 200 KB', InputFotoCentroEdu);
            return false;
        } else {
            //fotoCentro = await guardarAdjunto(InputFotoCentroEdu.files[0], InputFotoCentroEdu.files[0].type) || '';
